...
# Define the request functions for single site-locations

def pv_request(coordinates, year, token= None, capacity= 1.0,
               system_loss= 0.1, tracking= 0, tilt= 35, azim= 180):
    """
    Retrieve a series of PV output under the indicated specifications

    Args:
        coordinates (tuple): Coordinates as floats for location.
        year (string): Year for data retrieval.
        token (string, optional): String token for increased access limits. Default to None with limit 5 per day
        capacity (float, optional): Installed capacity at location. Default to 1.0.
        system_loss (float, optional): Loss of energy within the system. Default to 0.1.
        tracking (int, optional): Indicator if panels conduct tracking of sun. Default to 0.
        tilt (int, optional): Degree of tilt for the panels. Default to 35.
        azim (int, optional): Degree of azimuth for the the panels. Default to 180.
    """
    ...
    # Parse JSON to get a DataFrame of data and dict of metadata
    try:
...
def wind_request(coordinates, year, token= None, capacity= 1.0,
    ...
    """
    Retrieve a series of wind turbine output under the indicated specifications
    ...
    Args:
        ...
        token (string, optional): String token for increased access limits. Default to None with limit 5 per day
        capacity (float, optional): Installed capacity at location. Default to 1.0.
        height (int, optional): Height of turbine installed. Default to 100.
        turbine (str, optional): Model of turbine installed. Default to 'Vestas V80 2000'.
    """
...
# Define the request function for countries

def ninja_country(iso2_country, source, year, token):
    """
    
    Args:
        iso2_country (string): country name.
        source (string): type of power, "pv" for photovoltaic, "onshore" for wind
    ...
    #Generate URL based on country ISo2 code and source (pv/wind)
    if source == "pv":
        pv_adress = 'country_downloads/'+iso2_country+'/ninja_pv_country_'+iso2_country+'_merra-2_corrected.csv'
        url = api_base + pv_adress
    elif source == "onshore":
        wind_adress = 'country_downloads/'+iso2_country+'/ninja_wind_country_'+iso2_country+'_current-merra-2_corrected.csv'
        url = api_base + wind_adress
    ...
    ninja_data = pd.read_csv("temp.csv", header=1, skiprows=1)
    
    #Transform csv data
    ninja_data = ninja_data.loc[ninja_data["time"].str[:4].isin(year) , ["national"]]
    ninja_data.reset_index(drop=True, inplace=True) # reset index to 0
    ninja_data.reset_index(drop=False, inplace=True) # reset index to generate time step
    ninja_data.rename(columns={"national": "country_electricity",
                               "index": "time_step"}, 
                      inplace=True)
    
    return(ninja_data)
 
# Define helper functions for the loop
...
def prep_coord_list(coord_table):
    ...
    dep_list = coord_table["Department"].astype(str).to_list()
...
def limit_keeper(iterstart, iterend, limit_dict, burst_limit=6, sustained_limit=50):
    """Maintain the limits of the API
    
    Ensures that the calls to the API within a loop of requests do not exceed the minute limit or hourly limit.
    ...
    #Control time elapsed and iterations per minute
    ...
    #Control time elapsed and iterations per hour
...
def chain_limit_keeper(token, token_df, iterstart, iterend, limit_dict, burst_limit=6,
                       sustained_limit=50):
    """
    Maintain the limits of the API with chained tokens.
...
def ninja_flexreq(param_table, renewable, years, tokens=[], burst_limit=6, 
    ...
    coord_list, dep_list = prep_coord_list(param_table)
    ...
    while id < len(coord_list):
        ...
        if isinstance(years, tuple):
            print("Processing "+str(coord_list[id])+", "+str(dep_list[id])+", "+param_table.loc[id,"year"])
        else:
            print("Processing "+str(coord_list[id])+", "+str(dep_list[id]))
        ...
        if data.columns[0] == "invalid_coordinates":
            ...
            if isinstance(years, tuple): # if requests are broadcasted over years
                ...
                data.rename(columns={"electricity": "electricity"+dep_list[id]}, 
                            inplace= True) 
                
                # Merge into main dataset
                data_total = data_total.merge(data[["time_step","electricity"+dep_list[id]]],
                                              on="time_step", how= "outer", suffixes=("","_new"))
                
                # Fill in NA
                col_list = [col for col in data_total.columns if col.endswith('_new')]
                if len(col_list) > 0:
                    data_total["electricity"+dep_list[id]].fillna(data_total[col_list].iloc[:,0],
                                                                  inplace=True)
...
def ninja_aggregation(ninja_table, renewable, capacity_table, unit_divider= 1, 
                      source_type= ""):
    """
    Aggrate the results from requests function
